...
function checkForErrors()
{
    ...
    if(!isAnyFieldBlank){
      ...
      var isAnyErrorForSquareFootage = checkErrorsForSquareFootage();
      
      if(!(isAnyErrorForCity || isAnyErrorForPhoneNumber || isAnyErrorForSquareFootage))
      {
...
function checkErrorsForSquareFootage()
{
    var sqrft = importantInputs[7].value;
    console.log(sqrft);
    //true nese ka naj karakter tjeter perveq numrave , tipi osht number prandaj mund te kete numer negativ
    
    if(sqrft<=0)
    {
        console.log("Squarefootage keq");
        message[7].title="Please enter your home square footage";
        importantInputs[7].classList.add("error");
        return true;
    }
    else
    {
         message[7].title="";
         if(importantInputs[7].classList.contains("error"))
            importantInputs[7].classList.toggle("error");
         return false;
    }
